...
// Your web app's Firebase configuration
  // Your web app's Firebase configuration
var firebaseConfig = {
  apiKey: "AIzaSyA3Uck16zzCROd3fwo1-4bMot0TxM0UWXU",
  authDomain: "crud-jquery-firebase.firebaseapp.com",
  databaseURL: "https://crud-jquery-firebase-default-rtdb.firebaseio.com",
  projectId: "crud-jquery-firebase",
  storageBucket: "crud-jquery-firebase.firebasestorage.app",
  messagingSenderId: "865398950889",
  appId: "1:865398950889:web:5e285b9b819e829e299978"
};

// Initialize Firebase
firebase.initializeApp(firebaseConfig);
const db = firebase.database();
const coleccionProductos = db.ref().child('productos');

// ELIMINAR esta línea que no sirve:
// bodyProductos = $('#bodyProductos').val();

$('form').submit(function(e){
  e.preventDefault();
  
  let id = $('#id').val() || ''; // Asegurar que sea string vacío si no hay valor
  ...
  // Usar set en la referencia específica
  db.ref('productos/' + idFirebase).set(data);
...
function mostrarProductos({codigo, descripcion, cantidad}){
  ...
  <td>${descripcion}</td>
  <td>${cantidad}</td>
  <td><button class="btnEditar btn btn-secondary" data-toggle="tooltip" title="Editar">
  ${iconoEditar}</button>
  <button class="btnBorrar btn btn-danger" data-toggle="tooltip" title="Borrar">
  ${iconoBorrar}</button></td>
  `
}

// Para actualizar la tabla sin tener que recargar
//CHILD_ADDED
